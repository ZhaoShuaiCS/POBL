...
var logger = flogging.MustGetLogger("common.localdeliver")
...
type ChainManager interface {
	...
	LocalBroadcastChannelSupport(msg *cb.Envelope) (*cb.ChannelHeader, bool, ChannelSupport, error)
	GetChannelSupport(channelID string) (ChannelSupport,error)
}
...
func (h *Handler) deliverBlocks(ctx context.Context, srv *Server, envelope *cb.Envelope) (status cb.Status, err error) {
	...
	logger.Infof("zs:chainid:%s txid:%s get env",chdr.ChannelId,chdr.TxId)
	processor, err :=h.ChainManager.GetChannelSupport(chdr.ChannelId)
	if err != nil {
		logger.Infof("zs: [channel: %s] h.ChainManager.GetChannelSupport(chdr.ChannelId) with from: %s error: %s", chdr.ChannelId, addr, err)
		return cb.Status_BAD_REQUEST, err
	}
	err = processor.Order(envelope,521)
	logger.Infof("zs: Order ProcessMessage() get msg after send to order")
	if err != nil {
		logger.Infof("[channel: %s] Rejecting broadcast of normal message from %s with SERVICE_UNAVAILABLE: rejected by Order: %s", chdr.ChannelId, addr, err)
		return cb.Status_BAD_REQUEST, err
	}
