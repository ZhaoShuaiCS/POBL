...
func newChainSupport(
	...
	//}
	// consenter, ok := consenters["local"]
	// if !ok {
	// 	logger.Panicf("Error retrieving consenter of type: %s", "local")
	// }else{
	// 	logger.Infof("zs: new consenter, ok := consenters[\"local\"]")
	// }
	consenter, ok := consenters["etcdraft"]
	if !ok {
		logger.Panicf("Error retrieving consenter of type: %s", "etcdraft")
	}else{
		logger.Infof("zs: new consenter, ok := consenters[\"etcdraft\"]")
	}
