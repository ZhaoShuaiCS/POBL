...
//zs
func (r *Registrar) LocalBroadcastChannelSupport(msg *cb.Envelope) (*cb.ChannelHeader, bool, *ChainSupport, error) {
	chdr, err := utils.ChannelHeader(msg)
	if err != nil {
		return nil, false, nil, fmt.Errorf("could not determine channel ID: %s", err)
	}
...
func (r *Registrar) GetChannelSupport(channelID string) (*ChainSupport,error){

	cs := r.GetChain(channelID)
	if cs ==nil{
		logger.Infof("zs: GetChannelSupport error in chid:%s",channelID)
		return nil,errors.Errorf("zs: GetChannelSupport error in chid:%s",channelID)
	}
	return cs,nil
