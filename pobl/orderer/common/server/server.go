...
import (
	"fmt"
	"github.com/hyperledger/fabric/common/localdeliver"
	"io/ioutil"
	"os"
	"runtime/debug"
	"time"
...
//zs
type localdeliverSupport struct {
	*multichannel.Registrar
}

func (lds localdeliverSupport) GetChain(chainID string) localdeliver.Chain {
	chain := lds.Registrar.GetChain(chainID)
	if chain == nil {
		return nil
	}
	return chain
}
func (lds localdeliverSupport) LocalBroadcastChannelSupport(msg *cb.Envelope) (*cb.ChannelHeader, bool, localdeliver.ChannelSupport, error) {
	return lds.Registrar.LocalBroadcastChannelSupport(msg)
}

func (lds localdeliverSupport) GetChannelSupport(channelID string) (localdeliver.ChannelSupport,error) {
	return lds.Registrar.GetChannelSupport(channelID)
}
type server struct {
	...
	*multichannel.Registrar
	ldh   *localdeliver.Handler
}
func (s *server) LocalDeliver(srv ab.AtomicBroadcast_LocalDeliverServer) error {

	
	// if s.ldh.NumEnv<3{
	// 	s.ldh.NumEnv++
	// 	logger.Infof("zs:## Raft time[%v]",time.Now())
	// 	envelope, _ := srv.Recv()
	// 	s.bh.ProcessMessage(envelope,"gtz")
	// 	return nil
	// }else{
	s.ldh.NumEnv++
	logger.Infof("zs:## local time[%v]",time.Now())
	logger.Infof("zs: Starting new LocalDeliver handler")
	...
	localdeliverServer := &localdeliver.Server{
		PolicyChecker: localdeliver.PolicyCheckerFunc(policyChecker),
	...
	return s.ldh.Handle(srv.Context(), localdeliverServer)

	//}

}
// func (s *server) LocalDeliver(srv ab.AtomicBroadcast_LocalDeliverServer) error {
// 	logger.Infof("zs: Starting new LocalDeliver handler time[%v]",time.Now())
// 	envelope, _ := srv.Recv()
// 	// if err == io.EOF {
// 	// 	return nil
// 	// }
...
func NewServer(
	...
	s := &server{
		...
		debug:     debug,
		Registrar: r,
		ldh: localdeliver.NewHandler(
			localdeliverSupport{Registrar: r},
			timeWindow,
			mutualTLS,
			localdeliver.NewMetrics(metricsProvider),
			expirationCheckDisabled,
		),
	}
	return s
