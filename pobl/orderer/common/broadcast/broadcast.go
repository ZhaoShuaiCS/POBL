...
func (bh *Handler) ProcessMessage(msg *cb.Envelope, addr string) (resp *ab.BroadcastResponse) {
	...
	if !isConfig {
		...
		logger.Infof("zs: bh.Handle be txid:%s",chdr.TxId)
		logger.Infof("zs: [channel: %s] Broadcast is processing normal message from %s with txid '%s' of type %s", chdr.ChannelId, addr, chdr.TxId, cb.HeaderType_name[chdr.Type])
		...
		logger.Infof("zs: bh.Handle en txid:%s",chdr.TxId)
	} else { // isConfig
