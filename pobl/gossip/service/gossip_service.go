...
import (
	"fmt"
	localdeliverclient "github.com/hyperledger/fabric/core/localdeliverservice"
	"sync"
...
type GossipService interface {
	...
	//zs
	GetLocalDeliverService(chainID string )(localdeliverclient.DeliverService,error)
	//zs
}
//zs
//type GossipService interface {
...
//zs
type LocalDeliveryServiceFactory interface {
	// Returns an instance of delivery client
	LocalService(g GossipService, oac OrdererAddressConfig, msc api.MessageCryptoService) (localdeliverclient.DeliverService, error)
}
type localdeliveryFactoryImpl struct {
	isStaticLeader bool
}
func (d *localdeliveryFactoryImpl) LocalService(g GossipService, ec OrdererAddressConfig, mcs api.MessageCryptoService) (localdeliverclient.DeliverService, error) {
	return localdeliverclient.NewDeliverService(&localdeliverclient.Config{
	...
	}, localdeliverclient.ConnectionCriteria{
...
func (g *gossipServiceImpl) GetLocalDeliverService(chainID string )(localdeliverclient.DeliverService,error){
	ldc:=g.localdeliveryService[chainID]
	if g.deliveryService[chainID] == nil {
		return nil,errors.Errorf("zs: chainID:%s none ldc GetLocalDeliverClient",chainID)
	}
	return ldc,nil
}
//zs
...
type gossipServiceImpl struct {
	...
	localdeliveryService map[string]localdeliverclient.DeliverService
	localdeliveryFactory LocalDeliveryServiceFactory
	lock            sync.RWMutex
...
func InitGossipServiceCustomDeliveryFactory(
	...
	once.Do(func() {
		...
		gossipServiceInstance = &gossipServiceImpl{
			...
			localdeliveryService: make(map[string]localdeliverclient.DeliverService),
			localdeliveryFactory: &localdeliveryFactoryImpl{isStaticLeader: true},
...
func (g *gossipServiceImpl) InitializeChannel(chainID string, oac OrdererAddressConfig, support Support) {
	...
	if g.localdeliveryService[chainID] == nil {
		var err error

		for i,address:=range oac.Organizations{
			logger.Infof("zs: oac.Organizations index:%d address:%s \n", i,address)
		}
		for org,orderaddress :=range oac.AddressesByOrg{
			for i,address:=range orderaddress{
				logger.Infof("zs: oac.AddressesByOrg org:%s index:%d address:%s \n", org,i,address)
			}
		}
		for i,address:=range oac.Addresses{
			logger.Infof("zs: oac.Addresses index:%d address:%s \n", i,address)
		} 
		for key,override:=range oac.AddressOverrides{
			logger.Infof("zs: oac.AddressOverrides key:%s address:%s \n", key,override.Address)
		}
		var localaddress []string
		localaddress=append(localaddress,oac.Addresses[0])

		localoac:=OrdererAddressConfig{
			Addresses: localaddress,
			AddressesByOrg: oac.AddressesByOrg,
			Organizations: oac.Organizations,
			AddressOverrides: oac.AddressOverrides,

		}

		for i,address:=range localoac.Organizations{
			logger.Infof("zs: localoac.Organizations index:%d address:%s \n", i,address)
		}
		for org,orderaddress :=range localoac.AddressesByOrg{
			for i,address:=range orderaddress{
				logger.Infof("zs: localoac.AddressesByOrg org:%s index:%d address:%s \n", org,i,address)
			}
		}
		for i,address:=range localoac.Addresses{
			logger.Infof("zs: localoac.Addresses index:%d address:%s \n", i,address)
		} 
		for key,override:=range localoac.AddressOverrides{
			logger.Infof("zs: localoac.AddressOverrides key:%s address:%s \n", key,override.Address)
		}

		g.localdeliveryService[chainID], err = g.localdeliveryFactory.LocalService(g, localoac, g.mcs)
		if err != nil {
			logger.Infof("zs: Cannot create delivery client, due to %+v", errors.WithStack(err))
			logger.Warningf("zs: Cannot create delivery client, due to %+v", errors.WithStack(err))
		}
	}
	...
	if g.localdeliveryService[chainID] != nil {
		...
		logger.Debug("This peer is configured to connect to ordering service for blocks delivery, channel", chainID)
		logger.Infof("zs: g.localdeliveryService[chainID].StartDeliverForChannel(chainID, support.Committer, func() {})")
		g.localdeliveryService[chainID].StartDeliverForChannel(chainID, support.Committer, func() {})
		//} else {
		//	logger.Debug("This peer is not configured to connect to ordering service for blocks delivery, channel", chainID)
		//}
	} else {
...
func (g *gossipServiceImpl) Stop() {
	...
	for chainID := range g.chains {
		...
		if g.localdeliveryService[chainID]!=nil{
			logger.Infof("zs: g.localdeliveryService[chainID].Stop() %s", chainID)
			g.localdeliveryService[chainID].Stop()
		}
	}
	g.gossipSvc.Stop()
}
