...
//zs
// func GetBytesProposalResponsePayload(hash []byte, response *peer.Response, result []byte, event []byte, ccid *peer.ChaincodeID, txnCount int64, txid string) ([]byte, error) {
...
// }
// func GetTxnCountFromEnvelopeMsg(env *common.Envelope) (int64, string) {
...
// 	return  GetTxnCountPayloads(tx.Actions[0])
// }
// func GetTxnResultFromEnvelopeMsg(env *common.Envelope) (int64, string,*peer.ChaincodeActionPayload, *peer.ChaincodeAction, error) {
// 	payl, err := GetPayload(env)
// 	if err != nil {
// 		return -1, "error",nil,nil,nil
// 	}
...
// 	return  GetTxnResult(tx.Actions[0])
// }
...
//zs0423
func GetActionAndTxidFromEnvelopeMsg(env *common.Envelope) (*common.ChannelHeader,*peer.ChaincodeAction, error) {
	payl, err := GetPayload(env)
	if err != nil {
		return nil,nil, errors.New("zs: GetActionAndTxidFromEnvelopeMsg payl, err := GetPayload(env)")
	}

	tx, err := GetTransaction(payl.Data)
	if err != nil {
		return nil,nil, errors.New("zs: GetActionAndTxidFromEnvelopeMsg tx, err := GetTransaction(payl.Data)")
	}

	if len(tx.Actions) == 0 {
		return nil,nil, errors.New("zs: GetActionAndTxidFromEnvelopeMsg len(tx.Actions) == 0")
	}

	_, respPayload, err := GetPayloads(tx.Actions[0])
	if err!=nil{
		return nil,nil, errors.New("zs: GetActionAndTxidFromEnvelopeMsg _, respPayload, err := GetPayloads(tx.Actions[0])")
	}

	if payl.Header == nil {
		return nil,nil, errors.New("zs: GetActionAndTxidFromEnvelopeMsg payl.Header == nil")
	}

	if payl.Header.ChannelHeader == nil {
		return nil,nil, errors.New("zs: GetActionAndTxidFromEnvelopeMsg payl.Header.ChannelHeader == nil")
	}
	chdr := &common.ChannelHeader{}
	err = proto.Unmarshal(payl.Header.ChannelHeader, chdr)

	if err != nil {
		return nil,nil, errors.New("zs: GetActionAndTxidFromEnvelopeMsg proto.Unmarshal(payl.Header.ChannelHeader, chdr)")
	}

	return chdr,respPayload, nil
}
//zs0423
