...
//zs
// func (pe *PluginEndorser) EndorseWithPlugin(ctx Context, txnCount int64, txid string) (*pb.ProposalResponse, error) {
...
// 	prpBytes, err := proposalResponsePayloadFromContext(ctx, txnCount, txid)
// 	if err != nil {
// 		endorserLogger.Warning("Endorsement with plugin for", ctx, " failed:", err)
...
//zs
// func proposalResponsePayloadFromContext(ctx Context, txnCount int64, txid string) ([]byte, error) {
...
// 	prpBytes, err := putils.GetBytesProposalResponsePayload(pHashBytes, ctx.Response, ctx.SimRes, ctx.Event, ctx.ChaincodeID, txnCount, txid)
// 	if err != nil {
