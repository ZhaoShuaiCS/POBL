...
//zs
func (s *SupportImpl) RETpsCount(txid string,chainid string )error{
	lgr := s.Peer.GetLedger(chainid)
	if lgr == nil {
		return errors.Errorf("Channel does not exist: %s", chainid)
	}
	lgr.RETpsCount(txid)
	return nil
}
func (s *SupportImpl) ETpsCount(txid string,chainid string )error{
	lgr := s.Peer.GetLedger(chainid)
	if lgr == nil {
		return errors.Errorf("Channel does not exist: %s", chainid)
	}
...
func (s *SupportImpl) LocalDeliver(txid string,chainid string, prop *pb.Proposal, pResp *pb.ProposalResponse ) error{
	lgr := s.Peer.GetLedger(chainid)
	if lgr == nil {
		return errors.Errorf("Channel does not exist: %s", chainid)
	}
	//return lgr.NewTxSimulator(txid)
	return lgr.LocalDeliver(txid,chainid,prop,pResp)
}
//zs
