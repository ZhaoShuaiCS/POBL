...
type Support interface {
	...
	//zs
	//EndorseWithPlugin(ctx Context, txnCount int64, txid string) (*pb.ProposalResponse, error)
	LocalDeliver(txid string,chainid string, prop *pb.Proposal, pResp *pb.ProposalResponse ) error
	ETpsCount(txid string,chainid string )error
	RETpsCount(txid string,chainid string )error
	//zs
...
func (e *Endorser) callChaincode(txParams *ccprovider.TransactionParams, version string, input *pb.ChaincodeInput, cid *pb.ChaincodeID) (*pb.Response, *pb.ChaincodeEvent, error) {
	...
	//zs
	// if txParams!=nil&&len(input.Args)>=2{

	// 	// for i,_:=range input.Args {
	// 	// 	endorserLogger.Infof("zs: txid:%s Arg[%d]:[%s]\n",txParams.TxID,i,string(input.Args[i]))
	// 	// }
	// 	if len(input.Args[1])>=6{
	// 		peerID:=string(input.Args[1])
	// 		peer:=peerID[:6]
	// 		endorserLogger.Infof("zs: txid:%s peer:%s peerID:%s \n",txParams.TxID,peer,peerID)
	// 		if peer=="CLIENT"{
	// 			txParams.PeerID=peerID
	// 			txParams.TxCount,_ = strconv.ParseInt(string(input.Args[2]),10,64)
	// 			endorserLogger.Infof("zs: txid:%s PeerID:%s txcount:%d\n",txParams.TxID,txParams.PeerID,txParams.TxCount)
	// 			// tempArg:=[][]byte{}
	// 			// lenArg:=len(input.Args)
	// 			// tempArg=append(tempArg,input.Args[0])
	// 			// i:=3
	// 			// for i<lenArg{
	// 			// 	tempArg=append(tempArg,input.Args[i])
	// 			// 	i++
	// 			// }
	// 			// input.Args=tempArg
	// 			// for i,_:=range input.Args {
	// 			// 	endorserLogger.Infof("zs: After txid:%s Arg[%d]:[%s]\n",txParams.TxID,i,string(input.Args[i]))
	// 			// }
	
	// 		}else{
	// 			endorserLogger.Infof("zs: txid:%s PeerID:not exist txcount: not exist\n",txParams.TxID)
	// 		}
	// 	}
	// }
	//zs
...
//zs
// func (e *Endorser) endorseProposal(_ context.Context, chainID string, txid string, signedProp *pb.SignedProposal, proposal *pb.Proposal, response *pb.Response, simRes []byte, event *pb.ChaincodeEvent, visibility []byte, ccid *pb.ChaincodeID, txsim ledger.TxSimulator, cd ccprovider.ChaincodeDefinition, txnCount int64) (*pb.ProposalResponse, error) {
...
// 		TxID:           txid,
// 	}
// 	return e.s.EndorseWithPlugin(ctx, txnCount, txid)
// }
//zs
...
// 	endorserLogger.Infof("zs:be ProcessProposal")
// 	go e.GOProcessProposal(ctx,signedProp)
// 	endorserLogger.Infof("zs:en ProcessProposal")
// 	return nil,nil
// }
func (e *Endorser) ProcessProposal(ctx context.Context, signedProp *pb.SignedProposal) (*pb.ProposalResponse, error) {
	...
	// err=e.s.RETpsCount(txid,chainID)
	// if err!=nil{
	// 	endorserLogger.Infof("zs: txid: %s err in e.s.RETpsCount err:%s",txid,err.Error())
	// }else{
	// 	endorserLogger.Infof("zs: txid: %s success in e.s.RETpsCount",txid)
	// }
	//tireen:=time.Now()
	...
	beccTime:=time.Now()
	 endorserLogger.Infof("zs: get in endorser txid:%s cid:%s version:%s channelid:%s time[%v]",
	 txParams.TxID,hdrExt.ChaincodeId.Name,hdrExt.ChaincodeId.Version,txParams.ChannelID,time.Now())

	// endorserLogger.Infof("zs:timetxid:%s zstime-E: be[%v] becc[%v] addr[%v] preprocess[%v] retps[%v] simu[%v] init[%v]",
	// txid,beTime,beccTime,addrti.Sub(beTime),tirebe.Sub(addrti),tireen.Sub(tirebe),beccTime.Sub(tireen),beccTime.Sub(beTime))
	...
	// 1 -- simulate
	//cd, res, simulationResult, ccevent,txnCount, err := e.SimulateProposal(txParams, hdrExt.ChaincodeId)
	//if txnCount==-1{
	//	endorserLogger.Infof("zs: txid: %s -1 error in local e.SimulateProposal",txid)
	//}else if txnCount==-2{
	//	endorserLogger.Infof("zs: txid: %s -2 error in other e.SimulateProposal",txid)
	//}else{
	//	endorserLogger.Infof("zs: txid: %s %d success e.SimulateProposal",txid,txnCount)
	//}
	cd, res, simulationResult, ccevent, err := e.SimulateProposal(txParams, hdrExt.ChaincodeId)

	//tis:=time.Now()
	//endorserLogger.Infof("zs:timetxid:%s zstime-E: SimulateP[%v] ",txid,tis.Sub(beccTime))
	...
	if chainID == "" {
		...
		//zs
		//pResp, err = e.endorseProposal(ctx, chainID, txid, signedProp, prop, res, simulationResult, ccevent, hdrExt.PayloadVisibility, hdrExt.ChaincodeId, txsim, cd, txnCount)
		//zs
	...
	//endorserLogger.Infof("zs:timetxid:%s zstime-E: EndorseP[%v] ",txid,zhTime.Sub(tis))

	// total failed proposals = ProposalsReceived-SuccessfulProposals
	e.Metrics.SuccessfulProposals.Add(1)
	success = true
	endorserLogger.Infof("zs: before e.s.LocalDeliver txid: %s ",txid)

	//zs
	endorserLogger.Infof("zs: endorser txid:%s cid:%s version:%s channelid:%s",txParams.TxID,hdrExt.ChaincodeId.Name,hdrExt.ChaincodeId.Version,txParams.ChannelID)
	// if hdrExt.ChaincodeId.Name!="lscc"&& hdrExt.ChaincodeId.Name!="qscc"&& hdrExt.ChaincodeId.Name!="cscc"{
	// 	endorserLogger.Infof("zs: in e.s.LocalDeliver txid: %s channelid:%s",txid,txParams.ChannelID)

	// 	// err=e.s.ETpsCount(txid,chainID)
	// 	// if err!=nil{
	// 	// 	endorserLogger.Infof("zs: txid: %s err in e.s.ETpsCount err:%s",txid,err.Error())
	// 	// }else{
	// 	// 	endorserLogger.Infof("zs: txid: %s success in e.s.ETpsCount",txid)
	// 	// }



	// 	//err=e.s.LocalDeliver(txid,chainID,prop,pResp)




	// 	//err=e.s.Broadcast(chainID,txid,prop,pResp)
	// 	// if err!=nil{
	// 	// 	endorserLogger.Infof("zs: txid: %s err in e.s.Broadcast(chainID,txid,prop,pResp) err:%s",txid,err.Error())
	// 	// }else{
	// 	// 	endorserLogger.Infof("zs: txid: %s success in e.s.Broadcast(chainID,txid,prop,pResp)",txid)
	// 	// }
	// }
	//zs
	enTime:=time.Now()
	if enTime.Sub(beTime).Seconds()>=0.005{
		endorserLogger.Infof("zs:timetxid:%s zstime-E: be[%v] becc[%v] zh[%v] en[%v] init[%v] proces[%v] deliver[%v] E[%v]E5ms\n",
		txid,beTime,beccTime,zhTime,enTime,beccTime.Sub(beTime),zhTime.Sub(beccTime),enTime.Sub(zhTime),enTime.Sub(beTime))
	}else{
		endorserLogger.Infof("zs:timetxid:%s zstime-E: be[%v] becc[%v] zh[%v] en[%v] init[%v] proces[%v] deliver[%v] E[%v]Eless5ms\n",
		txid,beTime,beccTime,zhTime,enTime,beccTime.Sub(beTime),zhTime.Sub(beccTime),enTime.Sub(zhTime),enTime.Sub(beTime))
	}
