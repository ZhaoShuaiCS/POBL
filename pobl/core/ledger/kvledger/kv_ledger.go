...
type kvLedger struct {
	...
	ETXCount              int64
	ETXLastCount          int64
	ETXLastTime		   time.Time
	RETXCount              int64
	RETXLastCount          int64
	RETXLastTime		   time.Time
}
...
func newKVLedger(
	...
	l.ETXCount=0
	l.ETXLastCount=0
	l.ETXLastTime=time.Now()
	l.RETXCount=0
	l.RETXLastCount=0
	l.RETXLastTime=time.Now()
...
//zs
func (l *kvLedger) SetLocalDeliverService(chainid string)error {
	return l.txtmgmt.SetLocalDeliverService(chainid)
}
func (l *kvLedger)  LocalDeliver(txid string,chainid string, prop *peer.Proposal, pResp *peer.ProposalResponse ) error{
	return l.txtmgmt.LocalDeliver(txid,chainid,prop,pResp)
}
func (l *kvLedger)  ETpsCount(txid string ){
	l.ETXCount++
	if l.ETXCount==1000{
		l.ETXLastCount=l.ETXCount
		l.ETXLastTime=time.Now()
	}
	if l.ETXCount>1000{
		tempti:=time.Now()
		if tempti.Sub(l.ETXLastTime).Seconds()>=1{
			logger.Infof("zs: EETpsCount Ledger txnum:%d lastti[%v] nowti[%v] subti[%v]  alltx:%d\n",l.ETXCount-l.ETXLastCount,l.ETXLastTime,tempti,tempti.Sub(l.ETXLastTime),l.ETXCount)
			l.ETXLastCount=l.ETXCount
			l.ETXLastTime=tempti
		}
	}
	return
}
func (l *kvLedger)  RETpsCount(txid string ){
	l.RETXCount++
	//logger.Infof("zs: REETpsCount Ledger txnum:%d txid:%s\n",l.RETXCount,txid)
	if l.RETXCount==1000{
		l.RETXLastCount=l.RETXCount
		l.RETXLastTime=time.Now()
	}
	if l.RETXCount>1000{
		tempti:=time.Now()
		if tempti.Sub(l.RETXLastTime).Seconds()>=1{
			logger.Infof("zs: REETpsCount Ledger txnum:%d lastti[%v] nowti[%v] subti[%v]  alltx:%d\n",
			l.RETXCount-l.RETXLastCount,l.RETXLastTime,tempti,tempti.Sub(l.RETXLastTime),l.RETXCount)
			l.RETXLastCount=l.RETXCount
			l.RETXLastTime=tempti
		}
	}
	return
}
...
func (l *kvLedger) CommitWithPvtData(pvtdataAndBlock *ledger.BlockAndPvtData, commitOpts *ledger.CommitOptions) error {
	...
	var ZSt time.Time
	ZSt=time.Now()
	var ZSh,ZSm,ZSs,ZSns int
	ZSh=ZSt.Hour()
	ZSm=ZSt.Minute()
	ZSs=ZSt.Second()
	ZSns=ZSt.Nanosecond()
	logger.Infof("zs#B h %d m %d s %d ns %d zs#M h %d m %d s %d ns %d zs#E",ZSh,ZSm,ZSs,ZSns,ZSh,ZSm,ZSs,ZSns)
	//zs.e
	enTime:=time.Now()
	logger.Infof("zs:zstime-V: be[%v] zh[%v] en[%v] v[%v] c[%v] a[%v]",beTime,zhTime,enTime,zhTime.Sub(beTime),enTime.Sub(zhTime),enTime.Sub(beTime))
	return nil
}
//zs
//func (l *kvLedger) CommitWithPvtData(pvtdataAndBlock *ledger.BlockAndPvtData, commitOpts *ledger.CommitOptions) error {
