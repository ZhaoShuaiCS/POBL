...
type LockBasedTxMgr struct {
	...
	//zs
	LocalDeliverService localdeliverclient.DeliverService
	//zs
...
//zs
func (txmgr *LockBasedTxMgr)SetLocalDeliverService(chainid string)error{
	var err error
	txmgr.LocalDeliverService,err = service.GetGossipService().GetLocalDeliverService(chainid)
	if err!=nil{
		logger.Infof("zs:chainid:%s err:%s",chainid,err.Error())
		return err
	}
	return nil
}
func (txmgr *LockBasedTxMgr) LocalDeliver(txid string,chainid string, prop *peer.Proposal, pResp *peer.ProposalResponse ) error{
	logger.Infof("zs: txmgr *LockBasedTxMgr) LocalDeliver chainid:%s txid:%s before Send ",chainid,txid)
	str,err:=txmgr.LocalDeliverService.Send(txid,chainid,prop,pResp)
	if err!=nil{
		logger.Infof("zs: chainid:%s txid:%s err in Send str:%s err:%s",chainid,txid,str,err)
		return err
	}else{
		logger.Infof("zs: chainid:%s txid:%s success in Send str:%s",chainid,txid,str)
		return nil
	}
}
//zs
