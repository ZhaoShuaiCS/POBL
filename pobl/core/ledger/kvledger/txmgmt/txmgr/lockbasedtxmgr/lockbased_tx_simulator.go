...
func newLockBasedTxSimulator(txmgr *LockBasedTxMgr, txid string) (*lockBasedTxSimulator, error) {
	...
	return &lockBasedTxSimulator{lockBasedQueryExecutor{helper, txid}, rwsetBuilder, false, false, false, false,false,-1}, nil
}
//zs
//func newLockBasedTxSimulator(txmgr *LockBasedTxMgr, txid string) (*lockBasedTxSimulator, error) {
//	rwsetBuilder := rwsetutil.NewRWSetBuilder()
...
//zs
func (s *lockBasedTxSimulator) GetIsBroadcast() bool  {
...
//zs
func (s *lockBasedTxSimulator) GetTxnCount()int64 {
	return s.TxnCount
}
func (s *lockBasedTxSimulator) BuildRWset(buildRwset *kvrwset.BuildRwset,ns string) {
	//s.rwsetBuilder.ShowReadWriteSet(ns,s.txid)


	//s.rwsetBuilder=rwsetutil.NewRWSetBuilder()
	for _,rkv := range buildRwset.ReadSet {
		//logger.Infof("zs: set read txid:%s i:%d rk:%s rv:%s isread:%t fromTxid:%s txcount:%d",s.txid,i,rkv.Key,string(rkv.Value),rkv.IsRead,rkv.Txid,buildRwset.TxnCount)
		s.rwsetBuilder.AddToReadSetResult(ns, rkv,s.txid)
	}
	if len(buildRwset.WriteSet)>0{
		//logger.Infof("zs: set s.IsBroadcast=true txid:%s ",s.txid)
		s.IsBroadcast=true
	}else{
		//logger.Infof("zs: set s.IsBroadcast=false txid:%s ",s.txid)
	}
	for _,wkv := range buildRwset.WriteSet {
		//logger.Infof("zs: set write txid:%s j:%d wk:%s wv:%s iswrite:%t isdelete:%t txcount:%d",s.txid,j,wkv.Key,string(wkv.Value),wkv.IsWrite,wkv.IsDelete,buildRwset.TxnCount)
		s.rwsetBuilder.AddToWriteSetResult(ns, wkv,s.txid)
	}
	s.TxnCount=buildRwset.TxnCount
}
//zs
