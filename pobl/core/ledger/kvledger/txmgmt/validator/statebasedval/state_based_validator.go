...
func (v *Validator) ValidateAndPrepareBatch(block *internal.Block, doMVCCValidation bool) (*internal.PubAndHashUpdates, error) {
	...
	for _, tx := range block.Txs {
		...
		var err error
		if validationCode, err = v.validateEndorserTX(tx.RWSet, doMVCCValidation, updates,tx.ID); err != nil {
			return nil, err
		}
		...
		if validationCode == peer.TxValidationCode_VALID {
			...
			logger.Infof("Block [%d] Transaction index [%d] TxId [%s] marked as valid by state validator", block.Num, tx.IndexInBlock, tx.ID)
			...
			logger.Infof("Block [%d] Transaction index [%d] TxId [%s] marked as invalid by state validator. Reason code [%s]",
				block.Num, tx.IndexInBlock, tx.ID, validationCode.String())
			//logger.Warningf("Block [%d] Transaction index [%d] TxId [%s] marked as invalid by state validator. Reason code [%s]",
			//	block.Num, tx.IndexInBlock, tx.ID, validationCode.String())
			ZSinvalid=ZSinvalid+1
		}

		ZSblocksize=ZSblocksize+1
	}
	logger.Infof("zs@B b %d v %d i %d zs@M b %d v %d i %d zs@E",ZSblocksize,ZSvalid,ZSinvalid,ZSblocksize,ZSvalid,ZSinvalid)
	return updates, nil
}
//zs
//func (v *Validator) ValidateAndPrepareBatch(block *internal.Block, doMVCCValidation bool) (*internal.PubAndHashUpdates, error) {
...
func (v *Validator) validateEndorserTX(
	txRWSet *rwsetutil.TxRwSet,
	doMVCCValidation bool,
	updates *internal.PubAndHashUpdates,txid string) (peer.TxValidationCode, error) {

	var validationCode = peer.TxValidationCode_VALID
	var err error
	//mvccvalidation, may invalidate transaction
	if doMVCCValidation {
		validationCode, err = v.validateTx(txRWSet, updates,txid)
	}
	return validationCode, err
}
func (v *Validator) validateTx(txRWSet *rwsetutil.TxRwSet, updates *internal.PubAndHashUpdates,txid string) (peer.TxValidationCode, error) {
	...
	for _, nsRWSet := range txRWSet.NsRwSets {
		...
		if valid, err := v.validateReadSet(ns, nsRWSet.KvRwSet.Reads, updates.PubUpdates,txid); !valid || err != nil {
			if err != nil {
...
//}
//zs
func (v *Validator) validateReadSet(ns string, kvReads []*kvrwset.KVRead, updates *privacyenabledstate.PubUpdateBatch,txid string) (bool, error) {
	for _, kvRead := range kvReads {
		if kvRead.GetIsRead()==true{
			if valid, err := v.validateKVRead(ns, kvRead, updates,txid); !valid || err != nil {
				return valid, err
			}
		}
	}
...
func (v *Validator) validateKVRead(ns string, kvRead *kvrwset.KVRead, updates *privacyenabledstate.PubUpdateBatch,txid string) (bool, error) {

	if updates.Exists(ns, kvRead.Key) {
		updatesVersionedValue := updates.Get(ns, kvRead.Key)
		if updatesVersionedValue == nil {
			if kvRead.Value == nil{
				return true, nil
			}else{
				logger.Infof("zs: txid:%s   returned a rwset record: ns:%s  key:%s value: %s",txid, ns,kvRead.Key,string(kvRead.Value))
				logger.Infof("zs: txid:%s   compare a updates record:ns: %s key: %s value is nil",txid,
				ns, kvRead.Key)
				logger.Infof("zs: txid:%s   returned false")
				return false, nil
			}
		}else{
			if !bytes.Equal(updatesVersionedValue.Value,kvRead.Value) {
				if kvRead.Value !=nil {
					logger.Infof("zs: txid:%s   returned a rwset record: ns:%s  key:%s value: %s",txid, ns,kvRead.Key,string(kvRead.Value))
				}else{
					logger.Infof("zs: txid:%s   returned a rwset record: ns:%s  key:%s value: nil",txid, ns,kvRead.Key)
				}
				logger.Infof("zs: txid:%s   compare a updates record: ns: %s key: %s value: %s",txid,ns,kvRead.Key,string(updatesVersionedValue.Value))
				logger.Infof("zs: txid:%s   returned false")
				return false, nil
			}
			//logger.Infof("zs: txid:%s   returned true")
			return true, nil
		}

	} else{
		committedVersionedValue, err := v.db.GetState(ns, kvRead.Key)
		if err != nil {
			logger.Infof("zs: txid:%s  from commit returned false with err")
			return false, err
		}
		if committedVersionedValue == nil {
			if kvRead.Value == nil{
				return true, nil
			}else{
				logger.Infof("zs: txid:%s   compare a committted record:ns: %s key: %s value is nil",txid,
				ns, kvRead.Key)
				logger.Infof("zs: txid:%s   compare a rwset record:ns: %s key: %s value: %s",txid, ns,kvRead.Key,string(kvRead.Value))
				logger.Infof("zs: txid:%s   returned false")
				return false, nil
			}
		}else{
			if !bytes.Equal(committedVersionedValue.Value,kvRead.Value) {
				if kvRead.Value !=nil {
					logger.Infof("zs: txid:%s   returned a rwset record: ns:%s  key:%s value: %s",txid, ns,kvRead.Key,string(kvRead.Value))
				}else{
					logger.Infof("zs: txid:%s   returned a rwset record: ns:%s  key:%s value: nil",txid, ns,kvRead.Key)
				}
				logger.Infof("zs: txid:%s   compare a committed record:ns:%s key:%s value: %s",txid,ns,kvRead.Key,string(committedVersionedValue.Value))
				//???logger.Debugf("Version mismatch for key [%s:%s]. Committed version = [%#v], Version in readSet [%#v]",txid,
				//	ns, kvRead.Key, committedVersionedValue.Version, kvRead.Version)
				logger.Infof("zs: txid:%s   returned false")
				return false, nil
			}
			//logger.Infof("zs: txid:%s   returned true")
