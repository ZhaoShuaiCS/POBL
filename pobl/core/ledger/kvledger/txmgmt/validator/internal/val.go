...
func (u *PubAndHashUpdates) ApplyWriteSet(txRWSet *rwsetutil.TxRwSet, txHeight *version.Height, db privacyenabledstate.DB,txid string) error {
	...
	for compositeKey, keyops := range txops {
		if compositeKey.coll == "" {
			...
			//zs.b
			// if keyops.isDelete() {
			// 	logger.Infof(" zs: txid:%s  Updates.ApplyWriteSet :ns: %s key: %s  delete",txid, ns, key)
			// }else{
			// 	logger.Infof(" zs: txid:%s  Updates.ApplyWriteSet :ns: %s key: %s not delete",txid, ns, key)
			// }
			// if txHeight != nil{
			// 	logger.Infof(" zs: txid:%s  Updates.ApplyWriteSet :version: %d + %d ",txid,txHeight.BlockNum,txHeight.TxNum)
			// }
			// if keyops.metadata!=nil{
			// 	logger.Infof(" zs: txid:%s  Updates.ApplyWriteSet :metadata: %s",txid, string(keyops.metadata))
			// }
			// if keyops.value != nil {
			// 	logger.Infof(" zs: txid:%s  Updates.ApplyWriteSet :value: %s",txid, string(keyops.value))
