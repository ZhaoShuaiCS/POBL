...
//zs
func (b *RWSetBuilder) ShowReadWriteSet(ns string, txid string) {
	// nsPubRwBuilder := b.getOrCreateNsPubRwBuilder(ns)
	// for k,v := range nsPubRwBuilder.readMap{
	// 	logger.Infof("zs: txid:%s already ns:%s readk:%s v:%s",txid,ns,k,string(v.Value))
	// }
	// for k,v := range nsPubRwBuilder.writeMap{
	// 	logger.Infof("zs: txid:%s already ns:%s writek:%s v:%s",txid,ns,k,string(v.Value))
	// }
}
func (b *RWSetBuilder) AddToReadSetResult(ns string, readkv *kvrwset.KVRead,txid string) {
	nsPubRwBuilder := b.getOrCreateNsPubRwBuilder(ns)
	_,ok:=nsPubRwBuilder.readMap[readkv.Key]
	if ok==false{
		nsPubRwBuilder.readMap[readkv.Key] = readkv
		//logger.Infof("zs: txid:%s cc ns:%s rk:%s v:%s",txid,ns,readkv.Key,string(readkv.Value))
	}else{
		//logger.Infof("zs: txid:%s peer ns:%s rk:%s v:%s",txid,ns,readkv.Key,string(v.Value))
	}
...
func (b *RWSetBuilder) AddToWriteSetResult(ns string, writekv *kvrwset.KVWrite,txid string) {
	nsPubRwBuilder := b.getOrCreateNsPubRwBuilder(ns)
	_,ok:=nsPubRwBuilder.writeMap[writekv.Key]
	if ok==false{
		nsPubRwBuilder.writeMap[writekv.Key] = writekv
	//	logger.Infof("zs: txid:%s cc ns:%s wk:%s v:%s",txid,ns,writekv.Key,string(writekv.Value))
	}else{
		//logger.Infof("zs: txid:%s peer ns:%s wk:%s v:%s",txid,ns,writekv.Key,string(v.Value))
	}
}
//zs
...
func (b *RWSetBuilder) GetTxSimulationResults() (*ledger.TxSimulationResults, error) {
	...
	if pubSet != nil {
		if pubDataProto, err = pubSet.toProtoMsg(); err != nil {
			logger.Infof("zs: func (b *RWSetBuilder) GetTxSimulationResults() (")
			return nil, err
		}
	}
