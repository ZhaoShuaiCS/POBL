...
func (vm *DockerVM) createContainer(client dockerClient, imageID, containerID string, args, env []string, attachStdout bool) error {
	...
	//zs
	temp:=getDockerHostConfig()

	dockerLogger.Infof("zs docker: imageID:%s containerid:%s \n CgroupParent: %s Memory: %d  MemorySwap: %d    " +
		"MemorySwappiness: %d  OOMKillDisable: %t  CPUShares: %d    CPUSet: %s    CPUSetCPUs: %s   CPUSetMEMs: %s   " +
		" CPUQuota: %d    CPUPeriod: %d   BlkioWeight: %d   ",imageID,containerID,temp.CgroupParent,temp.Memory,temp.MemorySwap,temp.MemorySwappiness,temp.OOMKillDisable,
		temp.CPUShares,temp.CPUSet,temp.CPUSetCPUs,temp.CPUSetMEMs,temp.CPUQuota,temp.CPUPeriod,temp.BlkioWeight)
