...
import (
	"context"
	"fmt"
	"github.com/hyperledger/fabric/peer/localstatecommon"
...
var logger = flogging.MustGetLogger("localdeliveryClient")
...
type DeliverService interface {
	...
	Stop()
	Send(txid string,chainid string, prop *peer.Proposal, pResp *peer.ProposalResponse)(string,error)
}
...
func (d *deliverServiceImpl) StartDeliverForChannel(chainID string, ledgerInfo blocksprovider.LedgerInfo, finalizer func()) error {
	...
	if _, exist := d.deliverClients[chainID]; exist {
		...
		go d.launchSendChan(chainID)
	}
	return nil
}
//func (d *deliverServiceImpl) Send(txid string,chainid string, prop *peer.Proposal, pResp *peer.ProposalResponse) (string,error){
//	logger.Infof("zs: txid:%s before (d *deliverServiceImpl) Send(",txid)
//	signer,err:=localstatecommon.GetDefaultSigner()
//	if err != nil {
//		logger.Infof("zs: txid:%s error getting default signer",txid)
//		return txid,errors.WithMessage(err, "zs: error getting default signer")
//	}
//	env,err:=putils.LocalCreateSignedTx(prop, signer, pResp)
//	if err != nil {
//		logger.Infof("zs: txid:%s error getting env",txid)
//		return txid,errors.WithMessage(err, "zs: error env")
//	}
//	err=d.deliverClients[chainid].bclient.Send(env)
//	if err!=nil{
//		logger.Infof("zs: txid:%s error d.deliverClients[chainid].bclient.Send(env)",txid)
//		return txid,errors.WithMessage(err,"zs: error d.deliverClients[chainid].bclient.Send(env)")
//	}else{
//		logger.Infof("zs: txid:%s success in d.deliverClients[chainid].bclient.Send(env)",txid)
//	}
//	return txid,nil
//}
//zs
func (d *deliverServiceImpl) Send(txid string,chainid string, prop *peer.Proposal, pResp *peer.ProposalResponse) (string,error){
	...
	logger.Infof("zs: SendChan <- env before txid:%s ",txid)
	lench:= len(d.deliverClients[chainid].bclient.SendChan)

	d.deliverClients[chainid].bclient.SendChan <- &blocksprovider.SendEnv{
		Env:  env,
		Txid: txid,
		Betime: time.Now(),
	}
	d.deliverClients[chainid].bclient.NunpushSendchan++
	if lench>7500{
		logger.Infof("zs: SendChan <- env before txid:%s chanlen:%d !!!@@@!!!  maxbeNunpushSendchan:%d",txid,lench,d.deliverClients[chainid].bclient.NunpushSendchan)
	}else{
		logger.Infof("zs: SendChan <- env before txid:%s chanlen:%d  NunpushSendchan:%d",txid,lench,d.deliverClients[chainid].bclient.NunpushSendchan)
	}
	logger.Infof("zs: SendChan <- env end txid:%s ",txid)
	//err=d.deliverClients[chainid].bclient.Send(env)
	//if err!=nil{
	//	logger.Infof("zs: txid:%s error d.deliverClients[chainid].bclient.Send(env)",txid)
	//	return txid,errors.WithMessage(err,"zs: error d.deliverClients[chainid].bclient.Send(env)")
...
func (d *deliverServiceImpl) launchSendChan(chainID string) {
	d.lock.RLock()
	dc := d.deliverClients[chainID]
	d.lock.RUnlock()

	if dc == nil {
		logger.Info("zs: launchSendChan txn send for channel", chainID, "was stopped before block provider started")
		return
	}
	dc.bp.SendTxn()
}
