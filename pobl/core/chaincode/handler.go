...
type Handler struct {
	...
	//zs
	MsgMap map[string]*TxParamsMsg
	PeerTxCount map[string]int64
	MsgChan chan *TxParamsMsg
	//zs
}
//zs
type TxParamsMsg struct{
...
func (h *Handler) handleMessageReadyState(msg *pb.ChaincodeMessage) error {
	...
	case pb.ChaincodeMessage_BUILD_RWSET:
		go h.HandleTransaction(msg, h.HandleBuildRwset)
	default:
...
//zs
func (h *Handler) ProcessMsgChan() {
	for {
		txParamsMsg:= <-h.MsgChan
		if txParamsMsg.Msg==nil{
			return
		}else{
			if txParamsMsg.TxParams==nil ||  txParamsMsg.TxParams.PeerID!="CLIENT"{
				h.serialSendAsync(txParamsMsg.Msg)
			}else{
				chaincodeLogger.Infof("zs: ProcessMsgChan txid:%s PeerID:%s txcount:%d  BE \n", txParamsMsg.TxParams.TxID,txParamsMsg.TxParams.PeerID,txParamsMsg.TxParams.TxCount)
				txcount,isExist:=h.PeerTxCount[txParamsMsg.TxParams.PeerID]
				if isExist==false{
					h.PeerTxCount[txParamsMsg.TxParams.PeerID]=0
					txcount=h.PeerTxCount[txParamsMsg.TxParams.PeerID]
				}
				if txcount==txParamsMsg.TxParams.TxCount{
					chaincodeLogger.Infof("zs: ProcessMsgChan txid:%s PeerID:%s txcount:%d  mapcount:%d  serialSend\n", 
					txParamsMsg.TxParams.TxID,txParamsMsg.TxParams.PeerID,txParamsMsg.TxParams.TxCount,txcount)
					h.serialSendAsync(txParamsMsg.Msg)
					txcount++

					strtxcount :=strconv.FormatInt(txcount,10)
					tempkey:=txParamsMsg.TxParams.PeerID+"#"+strtxcount
					temptxParamsMsg,isExistTemp:=h.MsgMap[tempkey]
					for isExistTemp==true{
						chaincodeLogger.Infof("zs: for ProcessMsgChan txid:%s PeerID:%s txcount:%d  mapcount:%d  serialSend\n", 
						temptxParamsMsg.TxParams.TxID,temptxParamsMsg.TxParams.PeerID,temptxParamsMsg.TxParams.TxCount,txcount)
						h.serialSendAsync(temptxParamsMsg.Msg)
						delete(h.MsgMap,tempkey)
						txcount++
						...
						// 	txcount++
						// }else{
						// 	chaincodeLogger.Infof("zs: for ProcessMsgChan txid:not exist PeerID:%s txcount:%d  mapcount:%d ise[%v]\n", 
						// 	temptxParamsMsg.TxParams.PeerID,temptxParamsMsg.TxParams.TxCount,txcount,isExistTemp)
						// }
					}
					h.PeerTxCount[txParamsMsg.TxParams.PeerID]=txcount
				}else{
					chaincodeLogger.Infof("zs: ProcessMsgChan txid:%s PeerID:%s txcount:%d  mapcount:%d  wait add map\n", 
					txParamsMsg.TxParams.TxID,txParamsMsg.TxParams.PeerID,txParamsMsg.TxParams.TxCount,txcount)
					strTxCount := strconv.FormatInt(txParamsMsg.TxParams.TxCount,10)
					h.MsgMap[txParamsMsg.TxParams.PeerID+"#"+strTxCount]=txParamsMsg
				}
				chaincodeLogger.Infof("zs: ProcessMsgChan txid:%s PeerID:%s txcount:%d  EN \n", txParamsMsg.TxParams.TxID,txParamsMsg.TxParams.PeerID,txParamsMsg.TxParams.TxCount)
			}
		}
...
func (h *Handler) ProcessStream(stream ccintf.ChaincodeStream) error {
	...
	msgAvail := make(chan *recvMsg, 1000)
	...
	for {
		...
		case rmsg := <-msgAvail:
			...
			default:
				//beti:=time.Now()
				//chaincodeLogger.Infof("zs:timetxid:%s zstime-BERecv:[%v]", rmsg.msg.Txid, beti)
				...
				//enti:=time.Now()
				//chaincodeLogger.Infof("zs:timetxid:%s zstime-ENRecv:[%v]", rmsg.msg.Txid, enti)
...
//zs
func (h *Handler) HandleCompleted(msg *pb.ChaincodeMessage)  {
	chaincodeLogger.Infof("zs:txid:%s beHandleCompleted%v", msg.Txid,time.Now())
	buildRwset:= &kvrwset.BuildRwset{}
	err := proto.Unmarshal(msg.Payload, buildRwset)
	if err != nil {
		//return nil, errors.Wrap(err, "unmarshal failed")
	}
	collection := buildRwset.Collection

	if isCollectionSet(collection) {
		chaincodeLogger.Infof("zs: HandleBuildRwset not support private data")
		...
		txContext, errTxContext := h.isValidTxSim(msg.ChannelId, msg.Txid, "no ledger context")
		if errTxContext == nil {
			if buildRwset.TxnCount==-1{
				//chaincodeLogger.Infof("zs: TXS.buildrwset txid:%s error in cclocal", msg.Txid)
			}else{
				//chaincodeLogger.Infof("zs: TXS.buildrwset txid:%s succs in cclocal", msg.Txid)
				txContext.TXSimulator.BuildRWset(buildRwset,h.ChaincodeName())
			}

		}else{
			chaincodeLogger.Infof("zs: buildrwset error txid:%s  Failed to handle %s. error: %+v", msg.Txid, msg.Type, err)
		}
	}
	//if err != nil {
	...
	resMsg, _ := proto.Marshal(&pb.Response{Status: buildRwset.Status,Message: buildRwset.Message,Payload: buildRwset.Payload})
	msg.Payload=resMsg
	//resCCMsg := &pb.ChaincodeMessage{Type: pb.ChaincodeMessage_COMPLETED, Payload: resMsg, Txid: msg.Txid, ChaincodeEvent: msg.ChaincodeEvent, ChannelId: msg.ChannelId}
	h.Notify(msg)
	//chaincodeLogger.Infof("zs:txid:%s enHandleCompleted%v", msg.Txid,time.Now())
}
func (h *Handler) HandleBuildRwset(msg *pb.ChaincodeMessage, txContext *TransactionContext) (*pb.ChaincodeMessage, error) {
	buildRwset:= &pb.BuildRwset{}
	err := proto.Unmarshal(msg.Payload, buildRwset)
	if err != nil {
		return nil, errors.Wrap(err, "unmarshal failed")
	}
	...
	if isCollectionSet(collection) {
		...
		//res, err = txContext.TXSimulator.GetState(chaincodeName, getState.Key)
		//for readkey,readvalue := range buildRwset.ReadSet {
		//	chaincodeLogger.Infof("zs: readset txid:%s rk:%s rv:%s",msg.Txid,readkey,string(readvalue))
		//}
		//for writekey,writevalue := range buildRwset.WriteSet {
		//	chaincodeLogger.Infof("zs: readset txid:%s rk:%s rv:%s",msg.Txid,writekey,string(writevalue))
		//}
		str:=msg.Txid+"finish add RWresult"
		res=[]byte(str)
	}
	//if err != nil {
	//	return nil, errors.WithStack(err)
...
func (h *Handler) Execute(txParams *ccprovider.TransactionParams, cccid *ccprovider.CCContext, msg *pb.ChaincodeMessage, timeout time.Duration) (*pb.ChaincodeMessage, error) {
	...
	//beTime:=time.Now()
	//chaincodeLogger.Infof("zs:timetxid:%s zstime-cc: be[%v] ",txParams.TxID,beTime)
	...
	//zs
	// chaincodeLogger.Infof("zs:txid:%s before send h.MsgChan len:%d \n",txParams.TxID,len(h.MsgChan))
	// TxParamsMsg:=&TxParamsMsg{	
	// 	TxParams: txParams,
	// 	Msg:msg,
	// }
	// h.MsgChan<- TxParamsMsg
	// chaincodeLogger.Infof("zs:txid:%s end send h.MsgChan len:%d \n",txParams.TxID,len(h.MsgChan))
	//zs
	...
	//enTime:=time.Now()
	//chaincodeLogger.Infof("zs:timetxid:%s zstime-cc: be[%v] en[%v] cchandle[%v] ",
	//txParams.TxID,beTime,enTime,enTime.Sub(beTime))
	return ccresp, err
}
