...
func chatWithPeer(chaincodename string, stream PeerChaincodeStream, cc Chaincode) error {
	...
	fmt.Printf("zs: new handler ccname:%s",chaincodename)
	handler := newChaincodeHandler(stream, cc,chaincodename)
	defer stream.CloseSend()

	//go handler.LocalCommit()
	fmt.Printf("zs: buildLCchan default\n")
	handler.CommitChanMapLock.Lock()
	handler.CommitChanMap["default"]=&CommitChan{
		CommitC:make(chan *CommitMsg,5000),
		MsgMap: make(map[int64]*CommitMsg),
		TxCount:0,
		MaxMap:0,
	}
	go handler.LocalCommitChan(handler.CommitChanMap["default"])
	handler.CommitChanMapLock.Unlock()
	...
	for {
		...
		// if rmsg.msg.Type==pb.ChaincodeMessage_TRANSACTION{
		// 	txnumcount++
		// 	if txnumcount==1000{
		// 		cbetime=time.Now()
		// 		lastcount=txnumcount
		// 	}
		// 	if txnumcount>1000{
		// 		tempti:=time.Now()
		// 		if tempti.Sub(cbetime).Seconds()>=1{
		// 			fmt.Printf("zs: CTpsCount Receive txnum:%d lastti[%v] nowti[%v] subti[%v]  alltx:%d",txnumcount-lastcount,cbetime,tempti,tempti.Sub(cbetime),txnumcount)
		// 			cbetime=time.Now()
		// 			lastcount=txnumcount
		// 		}
		// 	}
...
func (stub *ChaincodeStub) init(handler *Handler, channelId string, txid string, input *pb.ChaincodeInput, signedProposal *pb.SignedProposal) error {
	...
	stub.LTS,_=NewLocalTxnSimulator(handler,txid)
	fmt.Printf("zs: stub txid:%s channelID:%s",txid,channelId)
	stub.LCChan=handler.CommitChanMap["default"]
...
func (stub *ChaincodeStub) GetState(key string) ([]byte, error) {
	chaincodenameAll:=stub.handler.ChaincodeName
	chaincodename:=chaincodenameAll[0:4]
	if chaincodename=="lscc"||chaincodename=="cscc"||chaincodename=="qscc"{
		//fmt.Printf("zs: txid:%s ns:%s stub.Getstate  system key:%s",stub.TxID,stub.ChannelId,key)
			collection := ""
			return stub.handler.handleGetState(collection, key, stub.ChannelId, stub.TxID)

	}else{
		v,err:=stub.LTS.GetState(key)
		if err!=nil{
			return v,err
		}else{
			return v,err
...
func (stub *ChaincodeStub) PutState(key string, value []byte) error {
	...
	if chaincodename=="lscc"||chaincodename=="cscc"||chaincodename=="qscc"{
		...
		return stub.LTS.SetState(key,value)
	}
...
func (stub *ChaincodeStub) DelState(key string) error {
	return stub.LTS.DeleteState(key)
}
//zs
//func (stub *ChaincodeStub) DelState(key string) error {
//	// Access public data by setting the collection to empty string
