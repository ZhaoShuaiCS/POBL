...
type CommitChan struct{
	...
	CommitC chan *CommitMsg
	MsgMap map[int64]*CommitMsg
	TxCount int64
	MaxMap int64
}
type Handler struct {
	...
	CommitChanMap map[string]*CommitChan
	CommitChanMapLock *sync.Mutex

	TxnCount int64
	ChaincodeName string
	CommitC chan *CommitMsg
	MsgMap map[string]*CommitMsg
	PeerTxCount map[string]int64
	MaxMap int64
	CTXCount              int64
	CTXLastCount          int64
	CTXLastTime		   time.Time

	CPTXCount              int64
	CPTXLastCount          int64
	CPTXLastTime		   time.Time

	CCTXCount              int64
	CCTXLastCount          int64
	CCTXLastTime		   time.Time
}

func (handler *Handler) GetTxCount() int64{
	txcount := handler.TxnCount
	handler.TxnCount=handler.TxnCount+1
	return txcount
}
...
func (handler *Handler) serialSend(msg *pb.ChaincodeMessage) error {
	beTime:=time.Now()
	fmt.Printf("zs:timetxid:%s zstime-Send: be[%v]\n",msg.Txid,beTime)
	handler.serialLock.Lock()
	defer handler.serialLock.Unlock()

	err := handler.ChatStream.Send(msg)
	enTime:=time.Now()
	fmt.Printf("zs:timetxid:%s zstime-Send: be[%v] en[%v] send[%v]\n",
	msg.Txid,beTime,enTime,enTime.Sub(beTime))
...
func newChaincodeHandler(peerChatStream PeerChaincodeStream, chaincode Chaincode,chaincodename string) *Handler {
	v := &Handler{
		ChatStream: peerChatStream,
		cc:         chaincode,
		LocalState: &LocalState{
			NameSpace:      "",
			LocalKeyValues: make(map[string]*KeyValue),
			KVLock:         make(map[string]*sync.Mutex),
			LKVMapLock:     new(sync.Mutex),
		},
		ChaincodeName: chaincodename,
		TxnCount: 0,
		CommitC: make(chan *CommitMsg,10000),
		MsgMap: make(map[string]*CommitMsg),
		PeerTxCount: make(map[string]int64),
		MaxMap:0,
		CTXCount:0,             
		CTXLastCount:0,         
		CTXLastTime:time.Now(),		  
		CPTXCount:0,             
		CPTXLastCount:0,         
		CPTXLastTime:time.Now(),	
		CCTXCount:0,             
		CCTXLastCount:0,         
		CCTXLastTime:time.Now(),	
		CommitChanMap:make(map[string]*CommitChan),
		CommitChanMapLock:new(sync.Mutex),
...
type CommitMsg struct{
	...
	OutChanTime time.Time
	Stub *ChaincodeStub
	Res pb.Response
	ErrC chan error
}
func (handler *Handler) ValidateAndCommit(msg *CommitMsg) {
	txid:=msg.Stub.TxID
	lts:=msg.Stub.LTS
	//fmt.Printf("zs: ValidateAndCommit txid:%s BE\n",txid)
	var nextStateMsg *pb.ChaincodeMessage
	flagCommit:=true
	for _,rkv :=  range lts.UNLockReadSets{
		if strings.Compare(rkv.StrValue,rkv.KeyValue.StrValue)!=0{
			//fmt.Printf("zs: ValidateAndCommit not same txid:%s rk:%s rkv:%s localkv:%s\n",txid,rk,rkv.StrValue,rkv.KeyValue.StrValue)
			flagCommit=false
			break
		}
	}
	if flagCommit==true{
		for rk,rkv :=  range lts.UNLockReadSets{
			lts.FinalReadSet=append(lts.FinalReadSet,&kvrwset.KVRead{Key: rk,Version: nil,Value: []byte(rkv.StrValue),IsRead: true,Txid: rkv.KeyValue.FromTxnid})
			
		}
		if len(msg.Stub.LTS.UNLockReadSets)>1{
			for wk,wkv:=range lts.UNLockWriteSets{
				_,ptxid,_:=handler.LocalState.SetUnLockKVForWrite(wk,wkv.BtValue,wkv.StrValue,txid)
				isDelete:=false
				if wkv.BtValue==nil{
					isDelete=true
				}
				lts.FinalWriteSet=append(lts.FinalWriteSet,&kvrwset.KVWrite{Key: wk,IsDelete: isDelete,Value: wkv.BtValue,IsWrite: true, Txid: ptxid})
			}
		}else{
			for wk,wkv:=range lts.UNLockWriteSets{
				_,ptxid,_:=handler.LocalState.SetUnLockKVForWrite(wk,wkv.BtValue,wkv.StrValue,txid)
		...
		res:=msg.Res
		payloadBytes, _ := proto.Marshal(&kvrwset.BuildRwset{ReadSet: lts.FinalReadSet,WriteSet: lts.FinalWriteSet,Collection: "",TxnCount: lts.TxidCount,
			Status: res.Status,Message: res.Message,Payload: res.Payload})
		enTime:=time.Now()
		fmt.Printf("zs:timetxid:%s zstime-LC: be[%v] enpro[%v] outchan[%v] enall[%v] PRO[%v] INCHAN[%v] VandC[%v] CCALL[%v]\n",
		txid,msg.BETime,msg.ProcessTime,msg.OutChanTime,enTime,msg.ProcessTime.Sub(msg.BETime),msg.OutChanTime.Sub(msg.ProcessTime),
		enTime.Sub(msg.OutChanTime),enTime.Sub(msg.BETime))
		nextStateMsg = &pb.ChaincodeMessage{Type: pb.ChaincodeMessage_COMPLETED, Payload: payloadBytes, Txid: txid, ChaincodeEvent: msg.Stub.chaincodeEvent, ChannelId: msg.Stub.ChannelId}
		handler.triggerNextState(nextStateMsg, msg.ErrC)
	}else{
		//fmt.Printf("zs: ValidateAndCommit txid:%s RExecute BE\n",txid)
		rebetime:=time.Now()
		msg.Stub.LTS,_=NewLocalTxnSimulator(handler,txid)
		msg.Res = handler.cc.Invoke(msg.Stub)
		lts=msg.Stub.LTS
		reentime:=time.Now()
		fmt.Printf("zs: ValidateAndCommit txid:%s RExecute EN\n",txid)
		...
		payloadBytes, _ := proto.Marshal(&kvrwset.BuildRwset{ReadSet: lts.FinalReadSet,WriteSet: lts.FinalWriteSet,Collection: "",TxnCount: lts.TxidCount,
			Status: res.Status,Message: res.Message,Payload: res.Payload})
		enTime:=time.Now()
		fmt.Printf("zs:timetxid:%s zstime-LC: be[%v] enpro[%v] outchan[%v] reend[%v] enall[%v] PRO[%v] INCHAN[%v] RE[%v] VandC[%v] CCALL[%v]\n",
		txid,msg.BETime,msg.ProcessTime,msg.OutChanTime,reentime,enTime,
		msg.ProcessTime.Sub(msg.BETime),msg.OutChanTime.Sub(msg.ProcessTime),reentime.Sub(rebetime),
		enTime.Sub(reentime),enTime.Sub(msg.BETime))
		nextStateMsg = &pb.ChaincodeMessage{Type: pb.ChaincodeMessage_COMPLETED, Payload: payloadBytes, Txid: txid, ChaincodeEvent: msg.Stub.chaincodeEvent, ChannelId: msg.Stub.ChannelId}
		handler.triggerNextState(nextStateMsg, msg.ErrC)
	}
}
func (handler *Handler) LocalCommitChan(cc *CommitChan) {
	fmt.Printf("zs:LocalCommit BE\n")
	for{
		msg:=<-cc.CommitC
		// txid:=msg.Stub.TxID
		// lts:=msg.Stub.LTS
		msg.OutChanTime=time.Now()
		handler.ValidateAndCommit(msg)
	}
}
...
func (handler *Handler) handleReady(msg *pb.ChaincodeMessage, errc chan error) error {
	...
	case pb.ChaincodeMessage_TRANSACTION:
		...
		fmt.Printf("zs: ChaincodeMessage_TRANSACTION: txid:%s ti[%v]\n", msg.Txid,time.Now())
		//chaincodeLogger.Infof("zs: ChaincodeMessage_TRANSACTION: txid:%s ti[%v]",msg.Txid,time.Now())
