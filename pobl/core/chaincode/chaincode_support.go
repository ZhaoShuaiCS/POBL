...
var chaincodeSupportLogger = flogging.MustGetLogger("ccSupportLogger")
...
func (cs *ChaincodeSupport) HandleChaincodeStream(stream ccintf.ChaincodeStream) error {
	handler := &Handler{
		...
		//zs
		MsgMap: make(map[string]*TxParamsMsg),
		PeerTxCount: make(map[string]int64),
		MsgChan: make(chan *TxParamsMsg,10),
		//zs
	}
...
func (cs *ChaincodeSupport) Execute(txParams *ccprovider.TransactionParams, cccid *ccprovider.CCContext, input *pb.ChaincodeInput) (*pb.Response, *pb.ChaincodeEvent, error) {
	...
	//zhTime:=time.Now()
	a,b,c:=processChaincodeExecutionResult(txParams.TxID, cccid.Name, resp, err)
	//enTime:=time.Now()
	// chaincodeSupportLogger.Infof("zs:timetxid:%s cctime-E: be[%v] zh[%v] en[%v] invoke[%v] processR[%v] ccSupport[%v]",
	// txParams.TxID,beTime,zhTime,enTime,zhTime.Sub(beTime),enTime.Sub(zhTime),enTime.Sub(beTime))
	return a,b,c
}
